...
from SHHS_Causal.SparseMax import Sparsemax, Entmax15
...
class TabNet(nn.Module):
    def __init__(
        ...
        self.post_embed_dim = self.representation_layers[0].post_embed_dim
    ...
    def forward(self, x_continuous, x_categorical, t):
        ...
        representation_x = self.representation_layers(x)
        x = torch.cat([representation_x, t], 1)

        steps_output, M_loss = self.tabnet(x)
        steps_output = torch.squeeze(steps_output)
        return representation_x, steps_output, M_loss
